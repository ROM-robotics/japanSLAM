...
class Submap:
    def __init__(self, atdS=0.0, cntS=0, cntE=-1, mps=None):
        ...
        self.mps = mps if mps else np.empty(0)  # Scan points in partial map Vector <lpoint2d>
...
class PointCloudMap:
    ...
    def __init__(
        ...
        # Overall map. Points after thinning
        ...
        # point group map using the for lattice table
        self.nntab = nntab if nntab else NNGridTable()
        # Cumulative mileage (ATD) [M]    
        self.atdThre = atdThre
        # Current cumulative mileage        
        self.atd = atd
        # Partial map
    ...
    # Additional LP of scanning groups
    ...
    # Generating the whole map. LP is faster to make local maps here
    def makeGlobalMap(self):
        ...
        for i in range(num):
            ...
            mps = submap.mps  # Part of the part of the map. Only the representative point
            num2 = len(mps)
            for j in range(num2):
