...
class DataAssociator:
    def __init__(self, curLps=None, refLps=None, baseLps=None, nntab=None):
        self.curLps = curLps if curLps else np.array([LPoint2D()])  # Current scanning group of scan
        self.refLps = refLps if refLps else np.array([LPoint2D()])  # A group of reference scan
        self.baseLps = baseLps if baseLps else np.empty(0)  # Store the reference scanning point. LS work
        self.nntab = nntab if nntab else NNGridTable()

    # Point of reference scanning RLPS as a pointer and put it in nntab
    def setRefBaseGT(self, rlps):
        self.nntab.clear()
        for i in range(len(rlps)):
            self.nntab.addPoint(rlps[i])  # Store as a pointer
    ...
    def findCorrespondenceGT(self, curScan, predPose):
        ...
        for i in range(len(curScan.lps)):
            ...
            # Recently seek nearby points due to a lattice table. Note that there is a distance threshold DTHRE in the lattice table
