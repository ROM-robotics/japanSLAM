...
class PointCloudMap:
    ...
    # Using the robot trajectory after adjusting the pose, Reconstruct the map
    def remakeMaps(self, newPoses):
        # Fix the position of the points in each part map
        for i in range(len(self.submaps)):
            submap = self.submaps[i]
            mps = submap.mps  # Part of the part of the map. It is a representative point except for the current map
            for j in range(len(mps)):
                mp = mps[j]
                idx = mp.sid  # Scan number of the point
                if idx >= len(self.poses):  # Unauthorized scan number (if there is a bug)
                    continue
                oldPose = self.poses[idx]  # Old robot position corresponding to MP
                newPose = newPoses[idx]  # New robot position corresponding to MP
                R1 = oldPose.Rmat
                R2 = newPose.Rmat
                lp1 = oldPose.relativePoint(mp)  # Converts MP into sensor coordinates with Oldpose
                lp2 = newPose.globalPoint(lp1)  # Convert to a map coordinate system after adjusting the pose with NewPose
                mp.x = lp2.x
                mp.y = lp2.y
                nx = R1[0, 0] * mp.nx + R1[1, 0] * mp.ny  # Converted the normal vector to the sensor coordinate system with Oldpose
                ny = R1[0, 1] * mp.nx + R1[1, 1] * mp.ny
                nx2 = R2[0, 0] * nx + R2[0, 1] * ny  # Converted the lone vector to the map coordinate system after adjusting the pose with NewPose
                ny2 = R2[1, 0] * nx + R2[1, 1] * ny
                mp.setNormal(nx2, ny2)
                self.submaps[i].mps[j] = mp
        self.makeGlobalMap()  # Generate the whole map and local map from partial maps
        num = len(self.poses)
