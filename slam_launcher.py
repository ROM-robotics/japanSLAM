...
class SlamLauncher:
    def __init__(self, startN=0, drawSkip=10, odometryOnly=False, ipose=None, lidarOffset=None, sreader=None, pcmap=None, sfront=None, mdrawer=None): # RasPiMouse drawskip 10
        ...
        self.mdrawer = mdrawer if mdrawer else MapDrawer()  # gnuplot  front end
    ...
    def run(self, inFile):
        self.mdrawer.setAspectRatio(1.0)  # The ratio of the X -...
        cnt = 0  # Processing logical t...
        if self.startN > 0:
            self.skipData(inFile, self.startN)  # startN Read the data to
        scan = Scan2D()
        #  Read one scan from t...
        eof = self.sreader.loadScan(inFile, cnt, scan)
        while eof is False:
            if self.odometryOnly:  # （SLAMより Memorial constructio...
                if cnt == 0:
                    self.ipose = scan.pose
                    self.ipose.calRmat()
                self.mapByOdometry(scan)
            else:
                self.sfront.process(scan)  # SLAM  Map construction wit...
                self.pcmap = self.sfront.pcmap
            if cnt % self.drawSkip == 0:  # drawSkip Draw the result ever...
                self.mdrawer.drawMapGp(self.pcmap)
            cnt = cnt + 1  # Logical time update
            eof = self.sreader.loadScan(inFile, cnt, scan)  #  Read the following s...
        ...
        if sys.platform != 'darwin':
            input()  # Wait for some kind o...

    # Read from the start...
    def skipData(self, inFile, num):
        scan = Scan2D()
        self.sreader.loadScan(inFile, 0, scan, skip=True)
        for i in range(num):  # num Read individuals
            self.sreader.loadScan(inFile, 0, scan, skip=True)

    # Building a map by od...
    def mapByOdometry(self, scan):
        ...
        for i in range(len(lps)):
            lp = lps[i]
            glp = LPoint2D()
            pose.globalPoint_io(lp, glp)  # Converted from senso...
            glps_list.append(glp)
        glps = np.asarray(glps_list)

        # pcmap Store the data in PC...
    ...
    def showScans(self, inFile):
        self.mdrawer.setRange(6)  # Drawing range. If th...
        self.mdrawer.setAspectRatio(1.0)  # The ratio of the X -...
        cnt = 0  #  Processing logical t...
        if self.startN > 0:
            self.skipData(inFile, self.startN)  # startN Read the data to
        ...
        eof = self.sreader.loadScan(inFile, cnt, scan)
        while eof is False:
            time.sleep(0.1)  #  The drawing interval...
